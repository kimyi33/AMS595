...
# Compatibility shim: older code expected torch_geometric.utils.scatter_
def scatter_(name, src, index, dim_size=None):
    """
    A minimal replacement for torch_geometric.utils.scatter_.

    This project only ever calls scatter_ with name='add', so we implement
    exactly that using torch_scatter.scatter_add.
    """
    if name != 'add':
        raise NotImplementedError(f"scatter_ only supports 'add', got {name}")

    # The old API expects: scatter_('add', src, index, dim_size=num_nodes)
    # scatter_add has signature: scatter_add(src, index, dim=0, dim_size=dim_size)
