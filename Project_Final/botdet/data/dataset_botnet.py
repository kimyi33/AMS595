...
class BotnetDataset(Dataset):
    ...
    def __init__(self, name='chord', root='data/botnet', split='train', graph_format='pyg', split_idx=None, add_nfeat_ones=True,
                 in_memory=True, use_integer_x=False):
        super().__init__()
        ...
        # NEW
        self.use_integer_x = use_integer_x 
    ...
    # ------- NEW FEATURELESS INTEGER INPUT -------
        #if self.use_integer_x:
        if 'x' not in graph_dict or graph_dict['x'] is None:
            num_nodes = graph_dict['num_nodes']
            graph_dict['x'] = np.arange(num_nodes, dtype='float32').reshape(-1, 1)
